...
#include "CLOUDEA/engine/exporters/paraview_exporter.hpp"
...
void ParaviewExporter::AddScalarField(const Eigen::VectorXd& scalar_field, const std::string &scalar_field_name)
{
    ...
    for (int i = 0; i != scalar_field.rows(); ++i) {
        scalars += std::to_string(scalar_field(i));
        scalars += "\n\t\t\t\t\t\t\t\t\t";
    }
...
void ParaviewExporter::CreatePvdAnimation(const std::string &vtu_files_dir, const int &files_number, const std::string &pvd_filename)
{
