...
#include "CLOUDEA/engine/options_configuration/config_manager.hpp"
...
ConfigManager::ConfigManager()
{
    this->description_.add_options()
            ...
            ("Loading.LoadName", boost_po::value<std::string>(),
             "Boundary name for loading condition application.")
            ("Loading.LoadTime", boost_po::value<double>(),
             "Time for loading application. Measure unit: [s]")
            ("Loading.LoadAxes", boost_po::value<std::string>(),
             "Triplet to set loading application on the x, y, z axes. Values: [1 | 0]")
            ("Loading.MaxDisplacement", boost_po::value<double>(),
             "Maximum displacement to applied due to the loading. Measure unit: [m]")
            ...
            ("DynamicRelaxation.EquilibriumTime", boost_po::value<double>()->default_value(10.),
             "Time for solution to reach equilibrium in dynamic relaxation. Measure unit: [s]")
            ...
            ("MTLED.SaveProgressSteps", boost_po::value<int>()->default_value(1),
...
void ConfigManager::ReadConfigFile(const std::string &config_filename)
{
    ...
    if (!input_stream.is_open()) {
        std::string error = "[CLOUDEA ERROR] cannot open CLOUDEA configuration file: " + config_filename;
...
std::string ConfigManager::PrintSampleFile()
{
    std::string sample = "#\n"
            ...
            "#\n\n\n"
            "######                  CLOUDEA V0.0.1 configuration file sample.                  ######\n"
            "\n"
            "# Section: Introduction\n"
            "# ---------------------\n"
            "\n"
            "# This text file demonstrates all the available arguments to launch and execute an CLOUDEA\n"
            ...
            "\n"
            "LoadTime = 1.0                                          # Time for loading application.\n"
            "                                                        # Measure unit: [s]\n"
            "\n"
            ...
            "\n"
            "MaxDisplacement = -0.02                                 # Maximum displacement to applied due to the loading.\n"
            "                                                        # Measure unit: [m]\n"
            "                                                        # Positive sign -> extension | Negative sign -> compression.\n"
            "\n\n"
            ...
            "                                                        # CLOUDEA adds an id to the filename according to the\n"
