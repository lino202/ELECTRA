    ...
    if (options.is_adaptive_) {
        ...
        switch (options.integ_points_per_tetra_) {
            ...
            case 5:
                ...
                break;
            default :
                std::string error = "[CLOUDEA ERROR] Invalid number of integration points requested. Admissible numbers: [1/4/5]";
...
void IntegPoints::Generate2(const TetraMesh &tetramesh, short num_per_elem)
{
    // Clear integration points and weights containers.
    this->coordinates_.clear();
    this->weights_.clear();
    ...
    switch (num_per_elem)
    {
        ...
        case 5:
            ...
            break;
        default:
            throw std::invalid_argument(Logger::Error("Cannot generate integration points "
                                        "for given number of points per element. Supported: [1 | 4 | 5] integration points per element."));
            break;
    }
...
void IntegPoints::LoadFromFile(const std::string & ip_file)
{
    // Clear integration points and weights containers.
    this->coordinates_.clear();
    this->weights_.clear();
    ...
    if (ip_file.empty()) {
        throw std::invalid_argument(Logger::Error("No filename was given to load integration points"));
    }

    //Open neighbours list file.
    std::ifstream ipoints(ip_file.c_str(), std::ios::in);

    // Check if neighbors list file opened successfully.
    if (!ipoints.is_open()) {
        std::string error = Logger::Error("Could not open the integration points file: \"") + ip_file + "\"";
    ...
    // Process file
    std::string line("");
    while (std::getline(ipoints, line)) {
        ...
        if (!line.empty() && line.find("*") == std::string::npos) {
            ...
            int id;
            double weight, ip_x, ip_y, ip_z;

            while(ss >> id >> weight >> ip_x >> ip_y >> ip_z) { 
                this->weights_.emplace_back(weight);
                this->coordinates_.emplace_back(Vec3<double>(ip_x,ip_y,ip_z)); 
            }
        }
                                             ...
                                             std::vector<Vec3<double> > &ip_coords, std::vector<double> &ip_weights)
{
    ...
    if (tet_nodes.size() != 4) {
        std::string error = "[CLOUDEA ERROR] Cannot generate integration points. The given element is not a tetrahedron.";
        throw std::invalid_argument(error.c_str());
    }
                                                 ...
                                                 const InfSupportDomain &support_dom)
{
    ...
    switch (options.tetra_divisions_) {
        case 2 :
            ...
            ip_num = 8*static_cast<int>(tetramesh.Elements().size());
            break;
        case 4 :
            // 4 x 4 ip per element.
            ip_num = 16*static_cast<int>(tetramesh.Elements().size());
            break;
        case 8 :
            // 8 x 4 ip per element.
            ip_num = 32*static_cast<int>(tetramesh.Elements().size());
            break;
        default:
            ip_num = 0;
    }
    ...
    for (const auto &elem : tetramesh.Elements()) {
        ...
        switch (options.tetra_divisions_) {
            ...
            case 8 :
                ...
                break;
            default :
                std::string error = "[CLOUDEA ERROR] Invalid number of tetrahedral divisions requested for"
                                            ...
                                            std::vector<Vec3<double> > &tet_ip_coords, std::vector<double> &tet_ip_weights)
{
    ...
    if (adaptive_level <= 0) {
        std::cout << "[CLOUDEA WARNING] The maximum adaptation level of integration points has been reached.\n";
        return;
    }
    ...
    int max_edge_id = std::distance(edges_sq.begin(), max_edge_sq_it);
                                           ...
                                           const std::vector<double> &ip_weights, const InfSupportDomain &support_dom)
{
    ...
    if (ip_coords.size() == 0) {
        std::string error = "[CLOUDEA ERROR] Cannot compute integration value. "
    ...
    auto neigh_ids = support_dom.CgalClosestNodesIdsTo(ip_coords);

    // Compute mmls shape functions.
    Mmls3d mmls3d;
    mmls3d.SetBasisFunctionType("linear");
    mmls3d.SetExactDerivativesMode(true);
    mmls3d.ComputeShFuncAndDerivs(tetramesh.Nodes(), ip_coords, neigh_ids, support_dom.InfluenceNodesRadiuses());
