...
void WeakModel3D::CreateIntegrationPoints(const IntegOptions &options, const InfSupportDomain &support_dom)
{
    // Create the model's integration points.
    this->integ_points_.Generate(this->tetramesh_, options, support_dom);
}

void WeakModel3D::LoadIntegrationPoints(const std::string &ip_file)
{
    // Create the model's integration points.
    this->integ_points_.LoadFromFile(ip_file);
}


void WeakModel3D::CreateIntegrationPoints2(short num_per_elem)
{
    // Create the model's integration points.
    this->integ_points_.Generate2(this->tetramesh_, num_per_elem);
}
...
void WeakModel3D::ComputeMass(const std::vector<double> &density, const std::vector<double> &time_steps, const double &max_time_step,
                              const std::vector< std::vector<int> > &support_nodes_ids, bool scaling)
{
    // Check if integration points weights are available.
    if (this->integ_points_.Weights().size() == 0) {
        std::string error = "[CLOUDEA ERROR] Cannot compute 3d weak model's mass. Integration points' weights are not initialized.";
        throw std::runtime_error(error.c_str());
    }
    ...
    // Check if model's grid node are available.
    if (this->grid_.Nodes().size() == 0) {
        std::string error = "[CLOUDEA ERROR] Cannot compute 3d weak model's mass. The model's grid nodes are not available.";
        throw std::runtime_error(error.c_str());
    }

    // Initialize the distributed mass to the model's grid points.
    this->mass_.clear();
    this->mass_.assign(this->grid_.Nodes().size(), 0.);

    // Initialize the distributed mass 3d matrix representaion.
    this->mass_matrix_ = Eigen::MatrixXd::Zero(this->grid_.Nodes().size(), 3);

    // Compute the distributed mass according to scale option.
    if (scaling) {

        // Set mass scaling conditional to true.
        this->is_mass_scaled_ = true;
        std::cout << "[CLOUDEA] Mass scaling: ON\n";
        ...
        std::cout << "[CLOUDEA] Maximum mass scaling factor: " << max_scale_factor << std::endl;
    }
    else {
        // Set mass scaling conditional to false.
        this->is_mass_scaled_ = false;
