...
#ifndef IMP_ENGINE_MESH_IO_ABAQUS_IO_HPP_
#define IMP_ENGINE_MESH_IO_ABAQUS_IO_HPP_
...
/**
 * \brief Class implementing input/output for mesh in Abaqus (*.inp) format. 
 * 
 * \tparam DIM The dimensions of the Abaqus mesh.
 * \tparam CELL_NODES The number of nodes in the elements of the Abaqus mesh.
 */
...
private:
    ...
    std::unordered_map<std::size_t, std::size_t> mapped_elem_ids_;      /**< The indices of the offsetted elements from the storage order and their offsets */

    std::vector<int> parts_startlines_;                                 /**< The indices of the starting line of the partition sets in the mesh filie */

    std::vector<int> nsets_startlines_;                                 /**< The indices of the starting line of the node sets in the mesh filie */
...
public:
     ...
     */
    inline virtual ~AbaqusIO();


    /**
     * \brief Read an abaqus mesh.
     *
     * The mesh to be readed should be in abaqus format (.inp).
     *
     ...
     * \return [void]
     */
    inline void SaveMesh(const std::vector<Vec<DIM,double>> &nodes, const std::vector<Cell<DIM,CELL_NODES>> &cells,
...
#endif // IMP_ENGINE_MESH_IO_ABAQUS_IO_HPP_

#include "IMP/engine/mesh_io/abaqus_io.tpp"
