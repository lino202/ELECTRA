...
void Gong2020m::SetDataMapping()
{
    using namespace Gng20mVar;
    using namespace Gng20mPrm;
    using namespace Gng20mCur;
    ...
    this->mapped_data_["Iion"] = static_cast<std::size_t>(Gng20mCur::Iion);
}
...
Gong2020m::Gong2020m()
{
    ...
    this->model_type_ = EpModelType::Gong2020m;
    this->dt_stable_ = 0.002;
    this->var_.resize(115, 0.);
...
void Gong2020m::Initialize(CellType cell_type)
{
    using namespace Gng20mVar;
    using namespace Gng20mPrm;

    //Initialize the model's data.
    this->var_.clear();           this->var_.resize(115, 0.); //this->var_.resize(116, 0.);
...
void Gong2020m::Compute(double v_new, double dt, double stim_current)
{
    using namespace Gng20mVar;
    using namespace Gng20mPrm;
    using namespace Gng20mCur;
...
void Gong2020m::ComputeBetaAdrenergicSignaling(double dt) 
{
    using namespace Gng20mVar;
    using namespace Gng20mPrm;
...
void Gong2020m::ComputeEffectiveFraction() 
{
    using namespace Gng20mVar;
    using namespace Gng20mPrm;
...
void Gong2020m::ComputeElectrophysiology(double v_new, double dt, double stim_current)
{
    using namespace Gng20mVar;
    using namespace Gng20mPrm;
    using namespace Gng20mCur;
    ...
    // // Update the m-gate using the Rush-Larson method computing the m-gate with the TenTusscher 2006 model's formula.
    // double aa_mTT2 = 1. / (1. + std::exp((-60. - v_new) / 5.));
    // double bb_mTT2 = 0.1 / (1. + std::exp((v_new + 35.) / 5.)) + 0.1 / (1. + std::exp((v_new - 50.) / 200.));
    // double tm = aa_mTT2 * bb_mTT2;
    // double mss = 1. / ((1. + std::exp((-56.86 - v_new) / 9.03))*(1. + std::exp((-56.86 - v_new) / 9.03)));
    // this->var_[m] = ALGORITHM::RushLarsen(mss, this->var_[m], dt, tm);
  
    // // Update the hs-gate using the Rush-Larson method computing the hs-gate with the TenTusscher 2006 model's formula. 
    ...
    // }
    // double ths = 1. / (aa_hTT2 + bb_hTT2);
    // double hss = 1. / ((1. + std::exp((v_new + 71.55) / 7.43)) * (1. + std::exp((v_new + 71.55) / 7.43)));
    ...
    // // Update the j-gate using the Rush-Larson method computing the j-gate with the TenTusscher 2006 model's formula.
    ...
    #ifdef BLOCK_CELL_CURRS
        this->cur_[INa] = (1.0-this->block_coeff_[INa]) * (this->prm_[GNa]*this->var_[m]*this->var_[m]*this->var_[m]*this->var_[h]*this->var_[j]*(v_new - ENa));
    #else
        this->cur_[INa] = (this->prm_[GNa]*this->var_[m]*this->var_[m]*this->var_[m]*this->var_[h]*this->var_[j]*(v_new - ENa));
    #endif
    /////////////////////////
    ...
    this->cur_[Gng20mCur::Iion] = this->cur_[INa]  + this->cur_[INaL] +  this->cur_[Ito] + this->cur_[ICaL] + this->cur_[ICaNa] + 
    ...
    this->var_[dvdt] = - (this->cur_[Gng20mCur::Iion] - stim_current);
...
//     double Ahf = 0.99;
//     double Ahs = 1.0-Ahf;
//     double dhf = (hss-this->var_[hf])/thf;
//     double dhs = (hss-this->var_[hs])/ths;
//     double h = Ahf*this->var_[hf]+Ahs*this->var_[hs];
//     double jss = hss;
//     double tj = 2.038+1.0/(0.02136*std::exp(-(v_new+100.6)/8.281)+0.3052*std::exp((v_new+0.9941)/38.45));
//     double dj = (jss-this->var_[j])/tj;

//     // gating CaMK-P
//     double hssp = 1.0/(1+std::exp((v_new+89.1)/6.086));
//     double thsp = 3.0*ths;
//     double dhsp = (hssp-this->var_[hsp])/thsp;
//     double hp = Ahf*this->var_[hf]+Ahs*this->var_[hsp];
//     double tjp = 1.46*tj;
//     double djp = (jss-this->var_[jp])/tjp;
...
//     double hP = Ahf*this->var_[hPf]+Ahs*this->var_[hPs];
//     double jPss = hPss; 
//     double djP = (jPss-this->var_[jP])/tj;

//     double hBPss = 1.0/(1.+std::exp((v_new+89.1 + VP_h)/6.086));
//     double thBPf = thf; 
//     double thBPs = thsp;
//     double dhBPf = (hBPss - this->var_[hBPf])/thBPf ;
//     double dhBPs = (hBPss - this->var_[hBPs])/thBPs ;

//     double hBP = Ahf*this->var_[hBPf]+Ahs*this->var_[hBPs];
//     double jBPss = hBPss ; 
//     double tjBP = tjp ;  
//     double djBP = (jBPss - this->var_[jBP])/tjBP ;
...
//     double mLss = 1.0/(1.0+std::exp((-(v_new+42.85))/5.264));
//     double tmL = tm;
//     double dmL = (mLss-this->var_[mL])/tmL;
//     double hLss = 1.0/(1.0+std::exp((v_new+87.61)/7.488));
//     double thL = 200.0;
//     double dhL = (hLss-this->var_[hL])/thL;
//     double hLssp = 1.0/(1.0+std::exp((v_new+93.81)/7.488));
//     double thLp = 3.0*thL;
//     double dhLp = (hLssp-this->var_[hLp])/thLp;
//     double fINaLp = (1.0/(1.0+this->prm_[KmCaMK]/CaMKa));
//     this->cur_[INaL] = (1.0-this->block_coeff_[INaL]) * (this->prm_[GNaL]*(v_new-ENa)*this->var_[mL]*((1.0-fINaLp)*this->var_[hL]+fINaLp*this->var_[hLp]));
...
//     double ass = 1.0/(1.0+std::exp((-(v_new-14.34))/14.82));
//     double ta = 1.0515/(1.0/(1.2089*(1.0+std::exp(-(v_new-18.4099)/29.3814)))+3.5/(1.0+std::exp((v_new+100.0)/29.3814)));
//     double da = (ass-this->var_[a])/ta;
//     double iss = 1.0/(1.0+std::exp((v_new+43.94)/5.711));
//     double delta_epi = 1.;
//     if (static_cast<int>(this->prm_[celltype]) == 1)
//         delta_epi -= (0.95/(1.0+std::exp((v_new+70.0)/5.0)));
    
//     double tiF = delta_epi * (4.562+1./(0.3933*std::exp((-(v_new+100.0))/100.0)+0.08004*std::exp((v_new+50.0)/16.59)));
//     double tiS = delta_epi * (23.62+1./(0.001416*std::exp((-(v_new+96.52))/59.05)+1.780e-8*std::exp((v_new+114.1)/8.079)));
//     double AiF = 1.0/(1.0+std::exp((v_new-213.6)/151.2));
//     double AiS = 1.0-AiF;
//     double diF = (iss-this->var_[iF])/tiF;
//     double diS = (iss-this->var_[iS])/tiS;
//     double i = AiF*this->var_[iF]+AiS*this->var_[iS];
//     double assp = 1.0/(1.0+std::exp((-(v_new-24.34))/14.82));
//     double dap = (assp-this->var_[ap])/ta;
//     double dti_develop = 1.354+1.0e-4/(std::exp((v_new-167.4)/15.89)+std::exp(-(v_new-12.23)/0.2154));
//     double dti_recover = 1.0-0.5/(1.0+std::exp((v_new+70.0)/20.0));
//     double tiFp = dti_develop*dti_recover*tiF;
//     double tiSp = dti_develop*dti_recover*tiS;
//     double diFp = (iss-this->var_[iFp])/tiFp;
//     double diSp = (iss-this->var_[iSp])/tiSp;
//     double ip = AiF*this->var_[iFp]+AiS*this->var_[iSp];
//     double fItop = (1.0/(1.0+this->prm_[KmCaMK]/CaMKa));
//     this->cur_[Ito] = (1.0-this->block_coeff_[Ito]) * (this->prm_[Gto]*(v_new-EK)*((1.0-fItop)*this->var_[a]*i+fItop*this->var_[ap]*ip));
...
//     double dss = 1.0/(1.0+std::exp((-(v_new+3.940))/4.230));
//     double td = 0.6+1.0/(std::exp(-0.05*(v_new+6.0))+std::exp(0.09*(v_new+14.0)));
//     double dd = (dss-this->var_[d])/td;
//     double fss = 1.0/(1.0+std::exp((v_new+19.58)/3.696));
//     double tff = 7.0+1.0/(0.0045*std::exp(-(v_new+20.0)/10.0)+0.0045*std::exp((v_new+20.0)/10.0));
//     double tfs = 1000.0+1.0/(0.000035*std::exp(-(v_new+5.0)/4.0)+0.000035*std::exp((v_new+5.0)/6.0));
//     double Aff = 0.6;
//     double Afs = 1.0-Aff;
//     double dff = (fss-this->var_[ff])/tff;
//     double dfs = (fss-this->var_[fs])/tfs;
//     this->prm_[f] = Aff*this->var_[ff]+Afs*this->var_[fs];
//     double fcass = fss; 
//     double tfcaf = 7.0+1.0/(0.04*std::exp(-(v_new-4.0)/7.0)+0.04*std::exp((v_new-4.0)/7.0));
//     double tfcas = 100.0+1.0/(0.00012*std::exp(-v_new/3.0)+0.00012*std::exp(v_new/7.0));
//     double Afcaf = 0.3+0.6/(1.0+std::exp((v_new-10.0)/10.0));
//     double Afcas = 1.0-Afcaf;
//     double dfcaf = (fcass-this->var_[fcaf])/tfcaf;
//     double dfcas = (fcass-this->var_[fcas])/tfcas;
//     double fca = Afcaf*this->var_[fcaf]+Afcas*this->var_[fcas];
//     double tjca = 75.0;
//     double djca = (fcass-this->var_[jca])/tjca;

//     // gating CaMK-P
//     double tffp = 2.5*tff; 
//     double dffp = (fss-this->var_[ffp])/tffp;
//     double fp = Aff*this->var_[ffp]+Afs*this->var_[fs];
//     double tfcafp = 2.5*tfcaf; 
//     double dfcafp = (fcass-this->var_[fcafp])/tfcafp;
//     double fcap = Afcaf*this->var_[fcafp]+Afcas*this->var_[fcas];
...
//     // PKA-P new steady state and variables
//     double dPss = 1.0/(1.0+std::exp((-(v_new+3.940+this->prm_[VP_d]))/4.230));
//     double ddP   = (dPss-this->var_[dP])/td;
//     double fPss = 1.0/(1.0+exp((v_new+19.58+this->prm_[VP_f])/3.696)); // ddP dfPf dfPs dfcaPf dfcaPs dfBPf dfcaBPf
//     double dfPf = (fPss-this->var_[fPf])/tff;
//     double dfPs = (fPss-this->var_[fPs])/tfs;
//     double fcaPss   = fPss; // steady state update, compute CDI gating again
//     double dfcaPf = (fcaPss-this->var_[fcaPf])/tfcaf;
//     double dfcaPs = (fcaPss-this->var_[fcaPs])/tfcas;
//     double fP   = Aff*this->var_[fPf]+Afs*this->var_[fPs];
//     double fcaP   = Afcaf*this->var_[fcaPf]+Afcas*this->var_[fcaPs];

//     // Both-P population takes on dP, for this->prm_[f] gate, takes ss from PKA and tau from CaMK (only fast component was modified)
//     double fBPss = fPss;
//     double dfBPf = (fBPss-this->var_[fBPf])/tffp;
//     double fBP = Aff*this->var_[fBPf]+Afs*this->var_[fPs]; // only fast component modified, slow component same as fPs
//     double fcaBPss = fcaPss;
//     double dfcaBPf = (fcaBPss-this->var_[fcaBPf])/tfcafp;  
//     double fcaBP = Afcaf*this->var_[fcaBPf]+Afcas*this->var_[fcaPs];
//     double fICaLp=(1.0/(1.0+this->prm_[KmCaMK]/CaMKa)); 
//     double fICaL_P = ICaL_P; 
//     double fICaL_BP = fICaLp*fICaL_P;
...
//     double Axrf = 1.0/(1.0+std::exp((v_new+54.81)/38.21));
//     double Axrs = 1.0-Axrf;
//     double dxrf = (xrss-this->var_[xrf])/txrf;
//     double dxrs = (xrss-this->var_[xrs])/txrs;
//     double xr = Axrf*this->var_[xrf]+Axrs*this->var_[xrs];
//     double rkr = 1.0/(1.0+std::exp((v_new+55.0)/75.0))*1.0/(1.0+std::exp((v_new-10.0)/30.0));
//     this->cur_[IKr] = (1.0-this->block_coeff_[IKr]) * (this->prm_[GKr]*std::sqrt(this->prm_[ko]/5.4)*xr*rkr*(v_new-EK));
...
//     double xs1ss = 1.0/(1.0+std::exp((-(v_new+11.60))/8.932));
//     double txs1 = 817.3+1.0/(2.326e-4*std::exp((v_new+48.28)/17.80)+0.001292*std::exp((-(v_new+210.0))/230.0));
//     double dxs1 = (xs1ss-this->var_[xs1])/txs1;
//     double xs2ss = xs1ss;
//     double txs2 = 1.0/(0.01*std::exp((v_new-50.0)/20.0)+0.0193*std::exp((-(v_new+66.54))/31.0));
//     double dxs2 = (xs2ss-this->var_[xs2])/txs2;
//     double KsCa = 1.0+0.6/(1.0+std::pow((3.8e-5/this->var_[cai]),1.4));
...
//     double txs1P = raw_taux1 + 817.3 + taux1factor / ((2.326*std::pow(10,-4)*std::exp((v_new+48.28)/17.8)) + 0.001292*std::exp(-(v_new+210.0)/230.0)) ;
//     double txs2P = txs2*raw_taux2 ;
//     double dxs1P = (xs1Pss-this->var_[xs1P])/txs1P;
//     double dxs2P = (xs2Pss-this->var_[xs2P])/txs2P;
    
//     // no CaMK-P in IKs
//     double IKs_NP = this->prm_[GKs]*KsCa*this->var_[xs1]*this->var_[xs2]*(v_new-EKs);
//     double IKs_PKA = GKsP*KsCa*this->var_[xs1P]*this->var_[xs2P]*(v_new-EKs);
//     double fIKs_PKA = IKs_P;
//     this->cur_[IKs] = (1.0-this->block_coeff_[IKs]) * ((1.-fIKs_PKA)*IKs_NP + fIKs_PKA*IKs_PKA);

//     //// calculate IK1
//     double xk1ss = 1.0/(1.0+std::exp(-(v_new+2.5538*this->prm_[ko]+144.59)/(1.5692*this->prm_[ko]+3.8115)));
//     double txk1 = 122.2/(std::exp((-(v_new+127.2))/20.36)+std::exp((v_new+236.8)/69.33));
//     double dxk1 = (xk1ss-this->var_[xk1])/txk1;
//     double rk1 = 1.0/(1.0+std::exp((v_new+105.8-2.6*this->prm_[ko])/9.493));
//     this->cur_[IK1] = (1.0-this->block_coeff_[IK1]) * (this->prm_[GK1]*std::sqrt(this->prm_[ko])*rk1*this->var_[xk1]*(v_new-EK));
...
//     double dJrelnp = (Jrel_inf-this->var_[Jrelnp])/tau_rel;
...
//     if (tau_relp < 0.001) tau_relp=0.001; 
//     double dJrelp = (Jrel_infp-this->var_[Jrelp])/tau_relp;
...
//     if (tau_relBP<0.001) tau_relBP=0.001; 
//     double dJrelBP = (Jrel_infBP-this->var_[JrelBP])/tau_relBP; 
...
//     this->var_[cansr] = ALGORITHM::ForwardEuler(this->var_[cansr], dt, dcansr); 
//     this->var_[cajsr] = ALGORITHM::ForwardEuler(this->var_[cajsr], dt, dcajsr); 
//     this->var_[m] = ALGORITHM::ForwardEuler(this->var_[m], dt, dm); 
//     this->var_[hf] = ALGORITHM::ForwardEuler(this->var_[hf], dt, dhf); 
//     this->var_[hs] = ALGORITHM::ForwardEuler(this->var_[hs], dt, dhs); 
//     this->var_[j] = ALGORITHM::ForwardEuler(this->var_[j], dt, dj); 
//     this->var_[hsp] = ALGORITHM::ForwardEuler(this->var_[hsp], dt, dhsp); 
//     this->var_[jp] = ALGORITHM::ForwardEuler(this->var_[jp], dt, djp); 
//     this->var_[mL] = ALGORITHM::ForwardEuler(this->var_[mL], dt, dmL);
//     this->var_[hL] = ALGORITHM::ForwardEuler(this->var_[hL], dt, dhL);
//     this->var_[hLp] = ALGORITHM::ForwardEuler(this->var_[hLp], dt, dhLp);
//     this->var_[a] = ALGORITHM::ForwardEuler(this->var_[a], dt, da);
//     this->var_[iF] = ALGORITHM::ForwardEuler(this->var_[iF], dt, diF);
//     this->var_[iS] = ALGORITHM::ForwardEuler(this->var_[iS], dt, diS);
//     this->var_[ap] = ALGORITHM::ForwardEuler(this->var_[ap], dt, dap);
//     this->var_[iFp] = ALGORITHM::ForwardEuler(this->var_[iFp], dt, diFp);
//     this->var_[iSp] = ALGORITHM::ForwardEuler(this->var_[iSp], dt, diSp);
//     this->var_[d] = ALGORITHM::ForwardEuler(this->var_[d], dt, dd); 
//     this->var_[ff] = ALGORITHM::ForwardEuler(this->var_[ff], dt, dff); 
//     this->var_[fs] = ALGORITHM::ForwardEuler(this->var_[fs], dt, dfs); 
//     this->var_[fcaf] = ALGORITHM::ForwardEuler(this->var_[fcaf], dt, dfcaf); 
//     this->var_[fcas] = ALGORITHM::ForwardEuler(this->var_[fcas], dt, dfcas); 
//     this->var_[jca] = ALGORITHM::ForwardEuler(this->var_[jca], dt, djca); 
//     this->var_[nca] = ALGORITHM::ForwardEuler(this->var_[nca], dt, dnca); 
//     this->var_[ffp] = ALGORITHM::ForwardEuler(this->var_[ffp], dt, dffp); 
//     this->var_[fcafp] = ALGORITHM::ForwardEuler(this->var_[fcafp], dt, dfcafp); 
//     this->var_[xrf] = ALGORITHM::ForwardEuler(this->var_[xrf], dt, dxrf); 
//     this->var_[xrs] = ALGORITHM::ForwardEuler(this->var_[xrs], dt, dxrs); 
//     this->var_[xs1] = ALGORITHM::ForwardEuler(this->var_[xs1], dt, dxs1); 
//     this->var_[xs2] = ALGORITHM::ForwardEuler(this->var_[xs2], dt, dxs2); 
//     this->var_[xk1] = ALGORITHM::ForwardEuler(this->var_[xk1], dt, dxk1); 
//     this->var_[Jrelnp] = ALGORITHM::ForwardEuler(this->var_[Jrelnp], dt, dJrelnp); 
//     this->var_[Jrelp] = ALGORITHM::ForwardEuler(this->var_[Jrelp], dt, dJrelp); 
//     this->var_[JrelBP] = ALGORITHM::ForwardEuler(this->var_[JrelBP], dt, dJrelBP); 
//     this->var_[CaMKt] = ALGORITHM::ForwardEuler(this->var_[CaMKt], dt, dCaMKt); 
//     this->var_[hPf] = ALGORITHM::ForwardEuler(this->var_[hPf], dt, dhPf); 
//     this->var_[hPs] = ALGORITHM::ForwardEuler(this->var_[hPs], dt, dhPs); 
//     this->var_[jP] = ALGORITHM::ForwardEuler(this->var_[jP], dt, djP); 
//     this->var_[hBPf] = ALGORITHM::ForwardEuler(this->var_[hBPf], dt, dhBPf); 
//     this->var_[hBPs] = ALGORITHM::ForwardEuler(this->var_[hBPs], dt, dhBPs); 
//     this->var_[jBP] = ALGORITHM::ForwardEuler(this->var_[jBP], dt, djBP); 
//     this->var_[dP] = ALGORITHM::ForwardEuler(this->var_[dP], dt, ddP); 
//     this->var_[fPf] = ALGORITHM::ForwardEuler(this->var_[fPf], dt, dfPf); 
//     this->var_[fPs] = ALGORITHM::ForwardEuler(this->var_[fPs], dt, dfPs); 
//     this->var_[fcaPf] = ALGORITHM::ForwardEuler(this->var_[fcaPf], dt, dfcaPf); 
//     this->var_[fcaPs] = ALGORITHM::ForwardEuler(this->var_[fcaPs], dt, dfcaPs); 
//     this->var_[fBPf] = ALGORITHM::ForwardEuler(this->var_[fBPf], dt, dfBPf); 
//     this->var_[fcaBPf] = ALGORITHM::ForwardEuler(this->var_[fcaBPf], dt, dfcaBPf); 
//     this->var_[xs1P] = ALGORITHM::ForwardEuler(this->var_[xs1P], dt, dxs1P); 
//     this->var_[xs2P] = ALGORITHM::ForwardEuler(this->var_[xs2P], dt, dxs2P); 
//     this->var_[JrelP] = ALGORITHM::ForwardEuler(this->var_[JrelP], dt, dJrelP);

// }


std::string Gong2020m::PrintVariables() const 
{
    using namespace Gng20mVar;
...
std::string Gong2020m::PrintParameters() const 
{
    using namespace Gng20mPrm;
...
std::string Gong2020m::PrintCurrents() const
{
    using namespace Gng20mCur;
    ...
    oss << "Iion = " << this->cur_[Gng20mCur::Iion];
    return oss.str();

}

#ifdef BLOCK_CELL_CURRS
    std::string Gong2020m::PrintBlockCoeffs() const
    {
        using namespace Gng20mCur;
